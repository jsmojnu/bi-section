document.getElementById('calculate').addEventListener('click', function(){
    ...
    console.log(bisection(a, b, x4, x3, x2, x1, xMukto));
})
...
function equation(x , x4, x3 , x2 , x1 , xMukto){
  
    return x4*x*x*x*x + x3 * x * x * x + x2 * x * x + x1 * x + xMukto;
...
function bisection(a, b, x4, x3, x2, x1, xMukto) {
    ...
    while ((b - a) >= epsilon) {
        c = (a + b)/2;
        if (equation(c, x4, x3, x2, x1, xMukto) === 0.0) {
            break;
        }
        else if (equation(c, x4, x3, x2, x1, xMukto) > 0) {
            b = c;
        }
        else {
            a = c;
